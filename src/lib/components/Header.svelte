<script>
  ...
  import { language, supportedLanguages, t, langFlags } from '$lib/langStore';
...
<div class="duration-5000 flex flex-col transition-colors ease-in-out">
  <header class="flex items-center justify-between bg-[#0b0b0b] p-2">
    <div
      class="mx-auto flex w-full max-w-screen-xl items-center justify-between p-2"
    >
    ...
    <nav class="mx-auto items-center space-x-4 md:flex">
          ...
          {#if isLanguageDropdownOpen}
            ...
            >
              {#each supportedLanguages as lang}
                <li>
                  <button
                    on:click={() => changeLanguage(lang.key)}
